...
tutorial = [
    ...
    },
    {
        ...
        "en": "The default port of the database RabbitMQ is 5672, the default port of Redis is 6379, and the default port of MongoDB is 27017."
              "Please consider whether to add firewall configuration. Among them, RabbitMQ and MongoDB use the default configuration password. Please check the .env file for modification."
    },
    {
        "cn": "请当心您的 .env 文件，其中包含了您的敏感信息。请不要将 .env 文件上传到公共仓库。",
        "en": "Please be careful with your .env file, which contains your sensitive information. Please do not upload the .env file to the public repository."
    },
    {
        "cn": "请当心您的 日志文件，其中包含了您的敏感信息。请不要将 日志文件上传到公共仓库。",
        "en": "Please be careful with your log file, which contains your sensitive information. Please do not upload the log file to the public repository."
    },
    {
    ...
    with console.status("[bold green]Working on tasks...[/bold green]") as status:
        ...
        while tutorial:
            ...
            for i in range(pre_step_stop):
                status.update(
                    f"[bold green]({index}/{tutorial_len})Remaining {pre_step_stop - i} seconds to next info... [/bold green] "
                )
                sleep(1)
