...
from llmkira.sender.util_func import (
    parse_command,
    is_command,
    is_empty_command,
    auth_reloader,
    ...
    async def run(self):
                    ...
                    if message.reply_to_message.document.file_size < 1024 * 1024 * 10:
                        _file.append(
                            await self.upload(
                                ...
                                uid=UserControl.uid_make(
                                    __sender__, message.from_user.id
                                ),
                            )
                        )
            logger.info(
        ...
        async def listen_clear_endpoint_command(message: types.Message):
            ...
            return await bot.reply_to(
                message,
                text=formatting.format_text(formatting.mbold(_tips), separator="\n"),
            ...
            try:
                ...
                return await bot.reply_to(
                    ...
                    text=formatting.format_text(
                        ...
                        formatting.mitalic(
                            "Format: /set_endpoint <openai_key>#<openai_endpoint>#<model>"
                        ),
        ...
        async def listen_bind_command(message: types.Message):
            ...
            try:
                router = Router.build_from_receiver(
                    receiver_channel=__sender__, user_id=message.from_user.id, dsn=_arg
                )
                _manager.add_router(router=router)
                router_list = _manager.get_router_by_user(
                    user_id=message.from_user.id, to_=__sender__
                )
            except Exception as e:
                logger.exception(e)
                return await bot.reply_to(
        ...
        async def listen_env_command(message: types.Message):
            ...
            _manager = EnvManager.from_meta(
                platform=__sender__, user_id=message.from_user.id
            )
            try:
        ...
        async def listen_clear_command(message: types.Message):
            RedisChatMessageHistory(
                session_id=f"{__sender__}:{message.from_user.id}", ttl=60 * 60 * 1
            ).clear()
            return await bot.reply_to(
                message,
                text=formatting.format_text(
            ...
            if trigger:
                if trigger == "allow":
                    return await create_task(
                        message, funtion_enable=__default_function_enable__
                    )
                if trigger == "deny":
                    return None
        ...
        async def handle_group_msg(message: types.Message):
            ...
            if trigger:
                if trigger.action == "allow":
                    return await create_task(
                        message, funtion_enable=trigger.function_enable
                    )
